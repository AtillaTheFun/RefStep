...
class GPIBThreadF(stuff.WorkerThread):
    ...
    def __init__(self, notify_window, EVT, param, data,start_time,OverideSafety,parent):
        ...
        for i in range(6):
            ...
            #this keeps the analysis sheets compatible with the normal input files.
...
class GPIBThreadDC(stuff.WorkerThread):
    ...
    def __init__(self, notify_window, EVT, param, data, start_time,
        ...
        stuff.WorkerThread.__init__(self, notify_window, EVT, param, data,
                                    start_time, OverideSafety, parent)
        self.inst_bus = param[0]  # visa or simulated visa2
        self.grid = param[1]
        self.voltmeter = param[2]
        self.dvm_range_col = param[3]
        self.Analysis_file_name = param[4]
        self.delayTime = param[5]
        self.nordgs = param[6]
        self.OverideSafety = OverideSafety
        self.MadeSafe = False
        time_bit = time.strftime("%Y.%m.%d.%H.%M.%S",time.localtime())
        log_file_name = 'log_files/DClog.'+time_bit+".txt"
        self.raw_file_name = 'data/DCdata/DCraw.'+time_bit
        self.wb = openpyxl.Workbook()
        self.sh = self.wb.active
        self.logfile = open(log_file_name,'w')
        self.ranges = 0
    ...
    def SafetyCheck(self):
        ...
        if state == 'clear':
            ...
            self.wait(SettleTime) #let instrumetns settle
            reading = float(self.com(self.voltmeter.read_instrument))
            if round(reading,2)!= 0.00:
                self.PrintSave('Terminals shorted, expect reading to be near 0, but it is '+str(reading))
                state = 'failed'
                return state
        if state == 'clear':
            if self.voltmeter.com["SingleMsmntSetup"] != 'SKIP':
